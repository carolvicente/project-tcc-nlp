...
data = [json.loads(post) for post in json.loads(open(DATA).read())]


all_posts = [post.get('text').lower().split()
             for post in data if post.get('text') is not None]

# Provides tf, idf, and tf_idf abstractions for scoring
tc = nltk.TextCollection(all_posts)


# Compute a term-document matrix such that td_matrix[(doc_title,url)][term]
...
for idx in range(len(all_posts)):
    ...
    data_ref = data[idx]
    if data_ref.get('title') is not None:
        doc_title = data[idx]['title']
    else:
        continue
    if data_ref.get('url') is not None:
        url = data[idx]['url']
    else:
        continue
...
for (title1, url1) in td_matrix.keys():
    ...
    for (title2, url2) in td_matrix.keys():
        ...
        if distances[(title1, url1)][(title2, url2)] < min_dist:
            (min_dist, most_similar) = (
                distances[(title1, url1)][(title2, url2)], (title2, url2))
    print '''Similar to %s (%s)\t, %s, (%s)\tscore %f''' % (title1, url1, most_similar[0], most_similar[1], 1-min_dist)
    print "*****************************"
